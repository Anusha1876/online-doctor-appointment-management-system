        ...
        <div className="container my-4" style={{ marginBottom: "50px" }}>
            ...
            <div>
                ...
                <table className="table table-bordered">
                    ...
                    <tbody>
                        {appointments.map((v, index) => (
                            <tr key={index}>
                                <td>{v.appointmentDate}</td>
                                <td>{v.appointmentTime}</td>
                                <td>{v.appointmentType}</td>
                                <td>{v.status === 'cancelled' ? 'Cancelled' : 'Scheduled'}</td>
                                <td>{v.patient_id && v.patient_id.patientId}</td> {/* Check if v.patient_id exists */}
                                <td>{v.patient_id && `${v.patient_id.firstName} ${v.patient_id.lastName}`}</td> {/* Check if v.patient_id exists */}
                            </tr>
                        ))}
                    </tbody>
                </table>
