...
function UpdatePatientDetails(){
    ...
    useEffect(() => {
        ...
        setData({patientId:patient.patientId,firstName:patient.firstName,lastName:patient.lastName,
            gender:patient.gender,dob:patient.dob,mobileNumber:patient.mobileNumber,bloodGroup:patient.bloodGroup,login_id:patient.login_id})
    },[]);
    ...
    const submitData=(e)=>{
        ...
        fetch("http://localhost:8080/updatepatientdetails",reqOptions)
            .then(resp=>resp.text())
        ...
        <h2 className='text-center'>Update Information </h2>
         ...
         <form method="POST">
            ...
            <div style={{ marginTop: '10px' }} className = "form-group">
                ...
                <input type="text" placeholder={data.bloodGroup} name="graduation" className="form-control" 
                    value={data.bloodGroup} onChange={changeHandler} disabled={true}/>
            </div >
