...
//	}
	...
	public DoctorTimeTable saveTimeTable(DoctorTimeTable dtt) {
	    try {
	        ...
	        DoctorTimeTable existingDtt = doctorTimeTableRepository.findWeekday( dtt.getDoctor_id(),dtt.getWeekday());
	...
	public List<LocalTime> getAppointmentsForDayByDoctorIdAndDay(int d, Date date) 
	{
		try {
			...
			DoctorTimeTable dtt = doctorTimeTableRepository.getBookedAppointmentsByDoctorIdandDate(d,day);
			if(dtt.getStatus().equals("available")) {
				...
				Doctor doc = doctorservice.getDoctorByDoctorId(d);
				List<Object> bookedAppointments = appointmentService.getBookedAppointmentsByDoctorIdandDate(doc, date);
